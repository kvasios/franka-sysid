...
from franka_sysid.eric_id.containers import dict_items_zip
from franka_sysid.eric_id.dair_pll_inertia import parallel_axis_theorem
from franka_sysid.eric_id.drake_torch_dynamics import (
...
from franka_sysid.eric_id.drake_torch_dynamics_test import (
    ensure_zero_grad,
    torch_uniform,
)
from franka_sysid.eric_id.drake_torch_sys_id import (
    DynamicsModel,
...
def add_model_panda_j7(plant):
    model_file = "models/panda_j7.urdf"
...
def add_model_panda(plant):
    model_file = "models/panda.dmd.yaml"
...
class Test(unittest.TestCase):
    ...
    def test_perturb_inertial_params(self):
        dyn_model, calc_mean_inertial_params_dist = self.make_dyn_model(add_model_panda)
        # Brief sanity checks.
    ...
    @torch.no_grad()
    def test_perturb_inertial_params(self):
        dyn_model, calc_mean_inertial_params_dist = make_dyn_model(add_model_panda)
        # Brief sanity checks.
        self.assertLess(calc_mean_inertial_params_dist(), 1e-6)
        # Ensure zero scale perturbation works as expected.
        perturb_inertial_params(dyn_model.inertial_params, perturb_scale=0.0)
    ...
    def test_bad_inertial_parameterizations(self):
        ...
        for bad_cls in bad_cls_list:
            ...
            # Fails as parameters are physically invalid
    ...
    def test_one_link_arm(self):
        self.check_sys_id(add_model_one_link_arm, loss_perturb_min=0.002, lr=1e-2)

    def test_2_iiwa(self):
        self.check_sys_id(add_model_iiwa, loss_perturb_min=0.002, lr=1e-2)
